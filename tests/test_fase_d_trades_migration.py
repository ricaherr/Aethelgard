...
Tests for:
1. Schema migration: trade_results → trades
2. Retrocompatibility: existing trades maintain LIVE/MT5/REAL defaults
3. New fields persistence: execution_mode, provider, account_type
4. Query filtering: get_trades() with execution_mode parameter
5. Default behavior: metrics (get_win_rate, etc.) default to LIVE trades
6. SHADOW trades: StrategyRanker can explicitly query SHADOW mode
...
class TestSchemaMigration:
    """Test schema migration from trade_results to trades."""
    
    def test_trades_table_exists(self, test_db):
        """Verify trades table exists after schema init."""
        cursor = test_db.cursor()
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='trades'")
        assert cursor.fetchone() is not None, "trades table does not exist"
    
    def test_trades_has_execution_mode_column(self, test_db, test_trade_id):
        ...
        test_db.commit()
        cursor.execute("SELECT execution_mode FROM trades WHERE id=?", (test_trade_id,))
        result = cursor.fetchone()
        assert result[0] == ExecutionMode.LIVE.value, f"Default execution_mode is not {ExecutionMode.LIVE.value}, got {result[0]}"
    
    def test_trades_has_provider_column(self, test_db, test_trade_id):
        ...
        test_db.commit()
        cursor.execute("SELECT provider FROM trades WHERE id=?", (test_trade_id,))
        result = cursor.fetchone()
        assert result[0] == Provider.MT5.value, f"Default provider is not {Provider.MT5.value}, got {result[0]}"
    
    def test_trades_has_account_type_column(self, test_db, test_trade_id):
        ...
        test_db.commit()
        cursor.execute("SELECT account_type FROM trades WHERE id=?", (test_trade_id,))
        result = cursor.fetchone()
        assert result[0] == AccountType.REAL.value, f"Default account_type is not {AccountType.REAL.value}, got {result[0]}"
    
    def test_trades_maintains_foreign_key_signal_id(self, test_db, test_trade_id, test_signal_id):
        """Verify FK constraint between trades.signal_id and signals.id."""
        cursor = test_db.cursor()
        # Create a signal first
        cursor.execute("""
            INSERT INTO signals (id, symbol, signal_type) 
            VALUES (?, ?, ?)
        """, (test_signal_id, 'EURUSD', 'BUY'))
        # Create a trade linked to the signal
        cursor.execute("""
            INSERT INTO trades (id, signal_id, symbol, entry_price, exit_price, profit)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (test_trade_id, test_signal_id, 'EURUSD', 1.08, 1.09, 100))
        test_db.commit()
        # Verify the trade was inserted
        cursor.execute("SELECT signal_id FROM trades WHERE id=?", (test_trade_id,))
        result = cursor.fetchone()
        assert result[0] == test_signal_id, "FK relationship broken"


class TestTradeResultPersistence:
    """Test that trades are saved with execution_mode, provider, account_type."""
    ...
    def test_save_trade_result_with_new_fields(self, storage, test_trade_id, test_signal_id):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM trades WHERE id=?", (test_trade_id,))
    ...
    def test_save_trade_result_backward_compatible(self, storage, test_trade_id, test_signal_id):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT execution_mode, provider, account_type FROM trades WHERE id=?", (test_trade_id,))
            row = dict(cursor.fetchone())
...
class TestQueryFiltering:
    ...
    def test_get_total_profit_defaults_to_live(self, storage):
        """get_total_profit() should default to LIVE trades only."""
        # Create LIVE trade
    ...
    def test_get_total_profit_with_shadow_filter(self, storage):
        """get_total_profit(execution_mode='SHADOW') should return only SHADOW trades."""
        storage.save_trade_result({
            'id': str(uuid.uuid4()),
            'symbol': 'EURUSD',
            'profit': 100,
    ...
    def test_get_win_rate_defaults_to_live(self, storage):
        """get_win_rate() should default to LIVE trades only."""
    ...
    def test_get_win_rate_with_shadow_filter(self, storage):
        """get_win_rate(execution_mode='SHADOW') should return only SHADOW trades."""
        storage.save_trade_result({'id': str(uuid.uuid4()), 'profit': 100, 'execution_mode': ExecutionMode.LIVE.value})
        storage.save_trade_result({'id': str(uuid.uuid4()), 'profit': -25, 'execution_mode': ExecutionMode.LIVE.value})
    ...
    def test_get_trades_public_method_exists(self, storage):
        """Verify get_trades() public method exists for unified query."""
        storage.save_trade_result({'id': str(uuid.uuid4()), 'profit': 100, 'execution_mode': ExecutionMode.LIVE.value})
        storage.save_trade_result({'id': str(uuid.uuid4()), 'profit': 50, 'execution_mode': ExecutionMode.SHADOW.value})
        
        # Test LIVE query
        live_trades = storage.get_trades(execution_mode=ExecutionMode.LIVE.value, limit=10)
        assert len(live_trades) == 1, "Should return 1 LIVE trade"
        
        # Test SHADOW query
        shadow_trades = storage.get_trades(execution_mode=ExecutionMode.SHADOW.value, limit=10)
        assert len(shadow_trades) == 1, "Should return 1 SHADOW trade"


class TestStrategyRankerIntegration:
    """Test StrategyRanker can query SHADOW trades for analysis."""
    
    def test_strategy_ranker_can_query_shadow_trades(self, storage):
        """StrategyRanker must be able to explicitly query SHADOW trades."""
        # Create mixed trades
        storage.save_trade_result({
            'id': str(uuid.uuid4()),
            'symbol': 'EURUSD',
        ...
        shadow_results = storage.get_trades(execution_mode=ExecutionMode.SHADOW.value, limit=100)
    ...
    def test_strategy_ranker_promotion_logic(self, storage):
        ...
        # Simulate SHADOW trades for a strategy
        storage.save_trade_result({'id': str(uuid.uuid4()), 'profit': 100, 'execution_mode': ExecutionMode.SHADOW.value})
        ...
        shadow_trades = storage.get_trades(execution_mode=ExecutionMode.SHADOW.value)
        ...
        assert len(shadow_trades) >= 3, "Should have at least 3 SHADOW trades"
        assert shadow_wr >= 0.5, "Should have >50% win rate for promotion"


class TestIntegrity:
    """Test data integrity and retrocompatibility with existing trades."""
    
    def test_existing_trade_history_not_contaminated(self, storage):
        """Historical trades (LIVE) should not be contaminated by SHADOW analysis."""
        ...
        for trade in old_trades:
            # These trades have NO execution_mode specified
            storage.save_trade_result(trade)
        
        # They should all default to LIVE
        live_count = len(storage.get_trade_results(limit=100))
        assert live_count == 10, "All historical trades should be LIVE"
        
        # SHADOW search should return nothing
        shadow_trades = storage.get_trades(execution_mode=ExecutionMode.SHADOW.value)
        assert len(shadow_trades) == 0, "Should not contaminate LIVE with SHADOW"
