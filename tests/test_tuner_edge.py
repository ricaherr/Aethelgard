...
SSOT: All config reads/writes go through StorageManager (Rule 14).
"""
import pytest
...
def temp_db(tmp_path):
    """Create isolated temp DB and seed initial dynamic_params."""
    db_path = tmp_path / "test_edge.db"
    storage = StorageManager(str(db_path))
    storage.update_dynamic_params(INITIAL_CONFIG)
    return storage


@pytest.fixture
def edge_tuner(temp_db):
    """Instantiate EdgeTuner (DB-only, no config_path)."""
    return EdgeTuner(storage=temp_db)


def test_tuner_becomes_conservative_after_losing_streak(temp_db, edge_tuner):
    """
    ...
    # Read new config from DB (SSOT)
    new_config = temp_db.get_dynamic_params()
...
def test_tuner_stability_within_target_range(temp_db, edge_tuner):
    """
    Si win_rate está cerca del target (50-60%), el tuner debe hacer ajustes mínimos
    """
    for i in range(20):
        is_winner = i % 2 == 0 and i < 22  # ~55% win rate
    ...
    # Read config BEFORE adjustment from DB
    old_config = temp_db.get_dynamic_params()
    old_atr = old_config["elephant_atr_multiplier"]

    adjustment = edge_tuner.adjust_parameters()

    # Read config AFTER adjustment from DB
    new_config = temp_db.get_dynamic_params()
    new_atr = new_config["elephant_atr_multiplier"]

    # Cambios deben ser mínimos (< 20% de variación)
