...
| Session Clock / MARKET GUARD | Reconocer ventanas Forex reales con DST por fecha sin drift entre módulos | Fuente unificada validada: 03:00 UTC activo (`sydney` + `tokyo`), overlap London-NewYork correcto con/sin DST y detector sin falso `ASIA` en ventana real de `NEW_YORK` | 🟢 OK | `pytest tests/test_market_session_service.py tests/test_session_state_detector.py tests/test_session_overlap_fix.py -q` = 34/34 PASS · smoke runtime `start.py`/`stop.py` 13-Abr-2026 sin falso cerrado | Mantener cobertura de fechas de transición DST |
